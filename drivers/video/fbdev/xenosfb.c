...
typedef struct {
	uint32_t enable;	 // D1GRPH_ENABLE
	uint32_t control;	// D1GRPH_CONTROL
	uint32_t lut_sel;	// D1GRPH_LUT_SEL
	uint32_t unk;		 // D1GRPH_SWAP_CNTL
	uint32_t base;		 // D1GRPH_PRIMARY_SURFACE_ADDRESS
	uint32_t unk_14;	 // 0x6114
	uint32_t secondary_base; // D1GRPH_SECONDARY_SURFACE_ADDRESS
	uint32_t unk_1C;	 // 0x611C
	uint32_t pitch;		 // D1GRPH_PITCH
	uint32_t offset_x;       // D1GRPH_SURFACE_OFFSET_X
	uint32_t offset_y;       // D1GRPH_SURFACE_OFFSET_Y
	uint32_t start_x;	// D1GRPH_X_START
	uint32_t start_y;	// D1GRPH_Y_START
	uint32_t width;		 // D1GRPH_X_END
	uint32_t height;	 // D1GRPH_Y_END
} ati_info;
...
static int __init xenonfb_probe(struct platform_device *dev)
{
	...
	printk("black: %d %d, %d %d\n", gfx[0x44 / 4], gfx[0x48 / 4],
	       gfx[0x4c / 4], gfx[0x50 / 4]);
	...
	// Finished using gfx, so unmap it.
	iounmap(gfx);
	...
	if (!request_mem_region(xenonfb_fix.smem_start, size_total,
				"xenonfb")) {
		printk(KERN_WARNING
...
// Shut down the early framebuffer driver (if enabled).
	...
	info->screen_base =
		ioremap(xenonfb_fix.smem_start, xenonfb_fix.smem_len);
	if (!info->screen_base) {
		printk(KERN_ERR
	...
	xenonfb_defined.yres_virtual =
		xenonfb_fix.smem_len / xenonfb_fix.line_length;
	...
	printk(KERN_INFO "xenonfb: pixclk=%ld left=%02x hsync=%02x\n",
	       ...
	       xenonfb_defined.left_margin, xenonfb_defined.hsync_len);
	...
	printk(KERN_INFO
	       ...
	       screen_info.rsvd_pos, screen_info.red_pos, screen_info.green_pos,
